...
def run():
    agent_name = get_agent_name("eurosciencejobs-discovery-worker")
    logger = get_logger(agent_name)
    api = make_api_client(timeout=30.0)

    http = HttpClient()
    scraper = EuroScienceJobsDiscovery(http)

    logger.info(f"Starting EuroScienceJobs sitemap discovery agent (name: {agent_name})")

    def cycle() -> bool:
        logger.info("Fetching all active job URLs from EuroScienceJobs sitemap category pages...")
        all_jobs = scraper.fetch_all_jobs_from_sitemap()

        if not all_jobs:
            logger.warning("No jobs retrieved from sitemap category pages")
            return False
