...
def run():
    agent_name = get_agent_name("naturecareers-discovery-worker")
    logger = get_logger(agent_name)
    api = make_api_client(timeout=30.0)

    http = HttpClient()
    scraper = NatureCareersDiscovery(http)

    logger.info(f"Starting Nature Careers XML Sitemap discovery agent (name: {agent_name})")

    def cycle() -> bool:
        logger.info("Fetching all active job URLs from Nature Careers XML sitemaps...")
