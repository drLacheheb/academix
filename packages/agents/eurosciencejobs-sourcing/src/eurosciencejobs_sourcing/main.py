...
def run():
    agent_name = get_agent_name("eurosciencejobs-sourcing-worker")
    ...
    logger.info(f"Starting EuroScienceJobs crawler sourcing agent (name: {agent_name})")
    ...
    def cycle() -> bool:
        ...
        if not pending_jobs:
            logger.info("All EuroScienceJobs jobs are fully scraped. Nothing to do.")
            return False
