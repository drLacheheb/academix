...
def run():
    agent_name = get_agent_name("abg-sourcing-worker")
    ...
    logger.info(f"Starting ABG crawler sourcing agent (name: {agent_name})")
    ...
    def cycle() -> bool:
        ...
        if not pending_jobs:
            logger.info("All ABG jobs are fully scraped. Nothing to do.")
            return False
