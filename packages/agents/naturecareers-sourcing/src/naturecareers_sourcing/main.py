...
def run():
    agent_name = get_agent_name("naturecareers-sourcing-worker")
    ...
    logger.info(f"Starting Nature Careers crawler sourcing agent (name: {agent_name})")
    ...
    def cycle() -> bool:
        ...
        if not pending_jobs:
            logger.info("All Nature Careers jobs are fully scraped. Nothing to do.")
            return False
