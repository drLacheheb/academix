...
def run():
    agent_name = get_agent_name("researchgate-discovery-worker")
    logger = get_logger(agent_name)
    config = get_config()
    api = make_api_client(timeout=30.0)

    http = HttpClient()
    scraper = ResearchGateDiscovery(http, max_pages=config["max_pages"])

    logger.info(f"Starting ResearchGate crawler discovery agent (name: {agent_name})")
